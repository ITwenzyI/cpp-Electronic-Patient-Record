...
bool Admin::checkInitialSetup() {
    if (!systemRepository().hasAssistantsDirectory() || !systemRepository().hasDoctorsDirectory()) {
        ...
        if (!systemRepository().hasAssistantsDirectory() &&
            !systemRepository().hasDoctorsDirectory()) {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 3: {
                ...
                data.firstName = firstName;
                data.lastName = lastName;
                const Result<void> createDoctorResult =
            ...
            case 4: {
                ...
                {
                    const Result<std::vector<std::string>> infoResult =
                        userProfileQueryService().getUserInfo(id);
                    if (!infoResult) {
                        ConsoleIO::printError(infoResult.error());
                        break;
                    }
            ...
            case 5: {
                ...
                data.firstName = firstName;
                data.lastName = lastName;
                const Result<void> createAssistantResult =
                ...
                std::cout << std::endl;
                std::cout << "Assistant: [" << firstName << " " << lastName
                          << "] successfully created!" << "\n";
                ConsoleIO::pauseSeconds(2);
                break;
            }
...
void Admin::exportUserData(const std::string& id) {
    ...
    if (!userRepository().exists(id)) {
        ConsoleIO::printError(
            {"USER_NOT_FOUND", "Invalid ID: " + id, ErrorSources::kUi, "Admin::exportUserData"});
        ConsoleIO::pauseSeconds(2);
        return;
    }
    ...
    if (content.empty()) {
        ConsoleIO::printError(
            {"READ_FAILED", "Failed to read file!", ErrorSources::kUi, "Admin::exportUserData"});
        return;
    }

    std::string folderName;
    if (!systemRepository().createUserBackupIfMissing(id, content, folderName)) {
        ConsoleIO::printError({"WRITE_FAILED", "Failed to create backup file.", ErrorSources::kUi,
