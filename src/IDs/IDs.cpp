...
int Patient_ID::get_patient_id() {
    ...
    if (file_in.is_open()) {
        ...
        std::cerr << "Faild to open and get ID from Patient!\n";
    }
    return id;
}
...
void Patient_ID::update_patient_id(int old_id) {
    ...
    if (file_out.is_open()) {
        ...
        std::cerr << "Faild to open and update ID from Patient!\n";
    }
}
...
int Assistant_ID::get_assistant_id() {
    ...
    if (file_in.is_open()) {
        ...
        std::cerr << "Faild to open and get ID from Assistant!\n";
    }
    return id;
}

void Assistant_ID::update_assistant_id(int old_id) {
    std::ofstream file_out("data/assistant_id.txt");
    if (file_out.is_open()) {
        file_out << old_id + 1;
        file_out.close();
    } else {
        std::cerr << "Faild to open and update ID from Assistant!\n";
    }
}
...
int Doctor_ID::get_doctor_id() {
    ...
    if (file_in.is_open()) {
        ...
        std::cerr << "Faild to open and get ID from Doctor!\n";
    }
    return id;
}

void Doctor_ID::update_doctor_id(int old_id) {
    std::ofstream file_out("data/doctor_id.txt");
    if (file_out.is_open()) {
        file_out << old_id + 1;
        file_out.close();
    } else {
        std::cerr << "Faild to open and update ID from Doctor!\n";
    }
}
