...
Result<std::vector<std::string>> PatientRecordQueryService::getPatientInfo(const std::string& patientId) const {
    if (!userRepository_.exists(patientId)) {
        return Result<std::vector<std::string>>::failure(
            ErrorCodes::kPatientNotFound, "Failed to read file!", ErrorSources::kApplication, "PatientRecordQueryService::getPatientInfo"
        );
    }
...
Result<std::vector<std::string>> PatientRecordQueryService::getAppointments(const std::string& patientId) const {
    ...
    if (appointments.empty()) {
        return Result<std::vector<std::string>>::failure(
            ErrorCodes::kAppointmentsNotFound, "Failed to read file!", ErrorSources::kApplication, "PatientRecordQueryService::getAppointments"
        );
    }
...
Result<std::vector<std::string>> PatientRecordQueryService::getMedications(const std::string& patientId) const {
    ...
    if (medications.empty()) {
        return Result<std::vector<std::string>>::failure(
            ErrorCodes::kMedicationsNotFound, "Failed to read file!", ErrorSources::kApplication, "PatientRecordQueryService::getMedications"
        );
    }
...
Result<std::vector<std::string>> PatientRecordQueryService::getRecords(const std::string& patientId) const {
    ...
    if (records.empty()) {
        return Result<std::vector<std::string>>::failure(
            ErrorCodes::kRecordsNotFound, "Failed to read file!", ErrorSources::kApplication, "PatientRecordQueryService::getRecords"
        );
    }
