...
Result<std::string> extractDoctor(const std::string& entry) {
    ...
    if (drStart == std::string::npos) {
        return Result<std::string>::failure(ErrorCodes::kDoctorNameNotFound,
            "Doctor name not found.", ErrorSources::kApplication,
            "AppointmentReviewService::extractDoctor");
    }
...
Result<std::vector<std::string>> AppointmentReviewService::loadRequests() const {
    if (!repository_.appointmentRequestsExists()) {
        return Result<std::vector<std::string>>::failure(ErrorCodes::kRequestsFileMissing,
            "Error: Could not open data/Appointments/requests.txt", ErrorSources::kApplication,
            "AppointmentReviewService::loadRequests");
    }
    return Result<std::vector<std::string>>::success(repository_.readAppointmentRequests());
}

Result<AppointmentDecisionOutcome> AppointmentReviewService::applyDecision(
    std::string& entry, const AppointmentDecision decision) const {
    ...
    if (!doctorResult) {
        return Result<AppointmentDecisionOutcome>::failure(doctorResult.errorCode(),
            doctorResult.errorMessage(), doctorResult.errorSource(), doctorResult.errorDetail());
    }

    if (!repository_.ensurePatientDirectory(patientId) ||
        !repository_.appendAppointment(
            patientId, dateTime + " - Dr. " + doctorResult.value() + " (confirmed)")) {
        // Keep review loop running for this recoverable write failure.
        return Result<AppointmentDecisionOutcome>::success(
            {false, true, "Error writing to data/Patients/" + patientId + "/appointments.txt"});
    }

    entry.replace(entry.find("pending"), 7, "confirmed");
    return Result<AppointmentDecisionOutcome>::success(
        {true, false, "Appointment confirmed and added to patient file."});
}
...
Result<void> AppointmentReviewService::saveRequests(const std::vector<std::string>& lines) const {
    if (!repository_.writeAppointmentRequests(lines)) {
        return Result<void>::failure(ErrorCodes::kWriteRequestsFailed,
            "\nFailed to update appointment file.", ErrorSources::kApplication,
            "AppointmentReviewService::saveRequests");
    }
